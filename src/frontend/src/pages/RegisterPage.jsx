...
export default function RegisterPage() {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      fireToast(error?.response?.data?.error || 'Registration failed', 'error');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <section className="mx-auto max-w-md px-4 py-16 sm:px-6 lg:px-8">
      <div className="rounded-3xl border border-slate-200 bg-white p-8 shadow-soft dark:border-slate-800 dark:bg-slate-900">
        ...
        <p className="mt-3 text-sm text-slate-500 dark:text-slate-400">Join NovaCart and begin shopping with a modern checkout experience.</p>
        ...
        <p className="mt-6 text-center text-sm text-slate-500 dark:text-slate-400">
          Already have an account? <Link to="/login" className="text-sky-600 hover:text-sky-700">Sign in</Link>
        </p>
      </div>
    </section>
  );
